...
switch ($_GET['data']) {
	...
	case 'update_baterai':
		...
		if ($kode_perangkat != '') {
			$sql = 'UPDATE device_smart_knock_lock SET KAPASITAS_BATERAI = ' . $kapasitas_baterai . ' WHERE KODE_PERANGKAT="' . $kode_perangkat . '"';
	...
	case 'insert_riwayat':
		...
		if ($kode_perangkat != '') {
			$perangkat = tampil("SELECT ID_PERANGKAT FROM device_smart_knock_lock WHERE KODE_PERANGKAT=" . "'$kode_perangkat'");
	...
	case 'getall':
		...
		$perangkat = tampil("SELECT ID_PERANGKAT FROM device_smart_knock_lock WHERE ID_MASTER=" . "$id_master");
		$id_perangkat = $perangkat['ID_PERANGKAT'];
	...
	case 'realtime':
		...
		$id_master = $data['id_master'];
		// echo $id_master;
		$sql = 'SELECT * FROM device_smart_knock_lock WHERE ID_MASTER = ' . $id_master;
		$result = mysqli_query($mysqli, $sql);
		if ($result) {
			$emparray = array();
			while ($row = mysqli_fetch_assoc($result)) {
	...
	case 'get_pressed':
		...
		$kode_perangkat = $data['kode_perangkat'];
		// $id_perangkat = 1;
		$sql = 'SELECT IS_PRESSED FROM device_smart_knock_lock WHERE KODE_PERANGKAT ="' . $kode_perangkat . '"';
		$result = mysqli_query($mysqli, $sql);
		if ($result) {
			$emparray = array();
			while ($row = mysqli_fetch_assoc($result)) {
